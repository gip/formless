---
name: web-ally
description: "Add or review a Web Ally layer on a website: WebMCP tools for semantic navigation and voice/text operation, a versioned model-site handshake, and a trusted agent system prompt. Use when making a web app agent-operable or defining its Web Ally protocol; do not treat this as a substitute for semantic HTML, keyboard support, or WCAG work."
---

# Web Ally

Implement Web Ally as an accessibility-oriented convention layered on the current WebMCP API. Preserve the site's visible, keyboard-accessible UI and reuse the same application services, validation, authorization, and audit paths that human interactions use.
...
- For website code, also read [references/implementation.md](references/implementation.md) and adapt [assets/web-ally-starter.ts](assets/web-ally-starter.ts).
- For review work, compare the implementation against the protocol invariants and verification checklist in those references. Report the distinction between WebMCP requirements and Web Ally conventions.

## Required outcome

Expose the smallest useful tool surface:

1. Register the eight core `web_ally.*` tools defined by the protocol when their capabilities exist.
...
3. Require `web_ally.handshake` before other Web Ally calls. Bind the session to the current origin and document, return an initial semantic page snapshot, and use a page revision to reject stale mutations.
...
- Web Ally augments rather than replaces accessible names, native controls, headings and landmarks, keyboard operation, focus order, status announcements, error association, and sufficient contrast.
...
When the browser does not implement the current draft, keep the Web Ally code feature-detected and provide a development-only adapter or console. Do not silently replace the public contract with an incompatible API.
